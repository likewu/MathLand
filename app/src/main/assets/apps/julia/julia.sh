...
if [ ! -f "$HOME"/.local/bin/julia ]; then
   ...
   wget https://julialang-s3.julialang.org/bin/linux/${cpuarch}/1.12/julia-1.12.1-linux-${cpuarch}.tar.gz
   tar -xvzf julia-1.12.1-linux-${cpuarch}.tar.gz
   ...
   rm julia-1.12.1-linux-${cpuarch}.tar.gz
   cd
   mkdir -p .local/bin
   #echo "export PATH=\$HOME/.local/bin:\$PATH" >> ~/.bashrc
   #. ~/.bashrc
   ln -s "$HOME"/julia-1.12.1/bin/julia "$HOME"/.local/bin
   #mkdir $HOME/.termux;
