...
class JuliaService : Service(), CoroutineScope {
    ...
    private fun updateSession(session: Session) = CoroutineScope(Dispatchers.Default).launch {
        UlaDatabase.getInstance(this@JuliaService).sessionDao().updateSession(session)
    }
    ...
    private fun startSshClient(session: Session) {
        ...
        connectBotIntent.data = Uri.parse("ssh://${session.username}@localhost:2022/#mathland")
