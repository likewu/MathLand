...
class GithubApiClient(
    ...
    private val baseUrl = "https://gitcode.com/leafcolor/MathLand/releases/download/"
    private val release_version = "-latest/"
    ...
    suspend fun getAssetsListDownloadUrl(repo: String): String = withContext(Dispatchers.IO) {
        ...
        return@withContext baseUrl + repo.substring(0, 1).toUpperCase() + repo.substring(1) + release_version + "${ulaFiles.getArchType()}-assets.txt"
    }
    ...
    suspend fun getAssetEndpoint(assetType: String, repo: String): String = withContext(Dispatchers.IO) {
        ...
        return@withContext baseUrl + repo.substring(0, 1).toUpperCase() + repo.substring(1) + release_version + "${ulaFiles.getArchType()}-$assetType"
    }
    ...
    private suspend fun queryLatestRelease(repo: String): ReleasesResponse = withContext(Dispatchers.IO) {
        ...
        if (!response.isSuccessful) {
            ...
            logger.addExceptionBreadcrumb(err)
            //throw err
            throw QueryLatestReleaseException("manifest.json is not found, but ok.")
        }
